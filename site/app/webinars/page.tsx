...
import { Section, Container, Card, Eyebrow, ArrowRight, HeroShade } from '@/components/ui'
  ...
  return (
    <Card as="article" className="group flex flex-col">
      <Link href={`/webinars/${item.slug}`} className="flex flex-1 flex-col">
        {date && <span className="eyebrow text-ink/50">{date}</span>}
        <h2 className="display-sm mt-3 text-balance">{item.title}</h2>
        {item.excerpt && (
          <p className="mt-3 line-clamp-3 text-ink/70">{item.excerpt}</p>
        )}
        <span className="mt-auto inline-flex items-center gap-1.5 pt-6 text-sm text-ink transition-colors group-hover:text-ink/60">
          View webinar
