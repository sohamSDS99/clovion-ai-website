 ...
 * accordion). Mirrors the Clovion vs Peec AI page's DeepDive, adapted for the
 * Otterly comparison and the LIGHT homepage palette.
 *
 * A topic rail selects one of the eight breakdown points; the selected point's
 * Clovion vs Otterly AI comparison is always shown on the right. One topic is
 * always open (defaults to the first) while only ever showing a single point's
...
export function DeepDive({ items }: { items: DeepDiveItem[] }) {
  ...
  const single = item.otterly.length === 0
      ...
      <div className="fx-explorer">
        ...
        <div className="fx-panel" key={active} role="tabpanel">
          ...
          <div className="fx-grid" data-single={single ? 'true' : 'false'}>
            ...
            {!single && <SideBlock label="Otterly AI" paragraphs={item.otterly} />}
          </div>
        </div>
      </div>
    </div>
