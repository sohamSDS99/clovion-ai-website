 ...
 * selected point's Clovion vs Searchable comparison is always shown on the
...
export function DeepDive({ items }: { items: DeepDiveItem[] }) {
  ...
  const single = item.searchable.length === 0
      ...
      <div className="fx-explorer">
        ...
        <div className="fx-panel" key={active} role="tabpanel">
          ...
          <div className="fx-grid" data-single={single ? 'true' : 'false'}>
            ...
            {!single && <SideBlock label="Searchable" paragraphs={item.searchable} />}
          </div>
        </div>
      </div>
    </div>
