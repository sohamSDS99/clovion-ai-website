...
/pricing                                       (LIGHT #FAF9F7 — components/pricing/*. Rebuilt 2026-07-07 (PR #41): launch banner, Monthly/Annually billing toggle with a DYNAMIC measured slider (pill sizes to the active label via refs+useLayoutEffect — PR #46), 3 tiers Starter $119 (launch $95) / Growth $319 (launch $255, "Most popular") / Enterprise Custom, "AI agent" note, agency band, 16-row compare table, 9-Q FAQ. Removed from all dark machinery.)
...
/compare/clovion-vs-peec-ai                    (added — Peec AI comparison)
...
- `components/Header.tsx` + `components/Footer.tsx` — LIGHT variants for non-dark routes. **"Features" is a `<button>` (not a Link)** — it has no href, exists only to open the mega-menu on hover/focus. Mobile nav renders Features as a non-link section header with its 6 children flat-listed below. **`Footer.tsx` FREE TOOLS column** — the first row is now **"Get Free Score" → `/free-ai-visibility-score`**, placed above llms.txt Generator (and the rest of the free-tools list). It's the canonical entry point from the footer. **Social icons are X/Twitter + LinkedIn only** — the GitHub icon was removed from both footers (PR #44, 2026-07-07). Keep light + dark social rows in sync.
- `components/HomeHeader.tsx` + `components/HomeFooter.tsx` — DARK variants. Same nav-button treatment for Features. **HomeFooter Company group is intentionally lighter than Footer** — only About / Customers / Affiliate Program (Compare / vs Profound / Profound alternatives were removed from the dark variant; light Footer keeps them). **`HomeFooter.tsx` FREE TOOLS column mirrors `Footer.tsx`** — first row is **"Get Free Score" → `/free-ai-visibility-score`**, above llms.txt Generator. Keep light + dark footer FREE TOOLS columns in sync.
...
- **Strict B&W brand book.** No purple, no chromatic accents. **Light theme is the default and the direction of travel** — `/`, `/pricing`, `/affiliate`, `/features/brand-perception`, `/features/brand-audit`, `/features/recommendation-engine`, and `/features/platform-coverage` are all LIGHT. Dark is now reserved for the remaining feature pages (ai-visibility-tracking, geo-improvement-suggestions, sentiment-analysis, fanout-query, ai-crawlability), `/free-ai-visibility-score`, `/customers`, `/about`, `/changelog`, and the content-prefix routes (see "Dark theme — broad coverage" for the exact `DARK_ROUTES` set). The homepage `/` keeps `ChatDemo`/`Loop`/`HomeCTA` as self-scoped dark bands on the light page. Don't add OR remove routes from the dark scope without per-page confirmation.
