...
| Lead-capture gate (modal opens automatically on `/free-ai-visibility-score` mount when `localStorage.clv_lead_captured !== '1'`; `POST /api/lead` 2xx sets the flag + unblocks the scan UI) | `components/free-score/FeatureContent.tsx` + `LeadCaptureModal` (locally mounted, controlled API: `{ open, onClose, onSuccess }`) | n/a (form-submit event handled by `LeadCaptureModal`) |
...
**Lead-capture gate lives on the feature, not the layout.** The `LeadCaptureModal` is **no longer mounted globally in `app/layout.tsx`** and the old document-level capture-phase click interceptor has been **removed** — "Get Free Score" CTAs across the site now navigate normally to `/free-ai-visibility-score` (no `preventDefault`, no synthetic click trap). The gate has moved to the destination page itself: `components/free-score/FeatureContent.tsx` mounts the modal locally, and on page mount it checks `localStorage.getItem('clv_lead_captured')`. If the flag is not `'1'`, the modal opens automatically and blocks the scan UI; returning visitors with the flag set skip the modal entirely and land directly on the scan form. Direct URL hits (no CTA upstream) are gated by the same mechanism — the gate is on the feature, not the click path. **Modal X-close → BLOCKED state**: dismissing without submitting disables the input and exposes a "Get Free Score" button below that reopens the modal (so the user can recover without reloading). **`LeadCaptureModal` API is now controlled**: `{ open, onClose, onSuccess }`. After a successful `/api/lead` 2xx, the modal writes `localStorage.clv_lead_captured = '1'` and invokes `onSuccess`, which unblocks the scan UI for the rest of the session.
...
- **CTAs are title-cased.** "Start Free Trial" → `https://app.clovion.ai/signup` (same-tab, in-product); "Get Free Score" → `/free-ai-visibility-score` (the **page itself** gates lead capture — every "Get Free Score" CTA across the site is now a plain navigation; the modal opens on the destination page if `localStorage.clv_lead_captured !== '1'`, and direct URL hits are gated the same way); "Log in" → `https://app.clovion.ai/login`; "Sign up" → `https://app.clovion.ai/signup`; "Talk to Sales" / "Talk to an Expert" → Calendly PopupWidget (see "Talk-to-Sales → Calendly" section). Button analytics matcher is case-insensitive (`text.toLowerCase().includes('free trial')`) so old lowercase still routes to `start_trial`/`get_free_score` events if it ever reappears. The Loop section's secondary CTA is "Talk to an Expert" (Calendly) — not "See the product". Sales CTAs ("Talk to Sales", "Talk to an Expert") are fine in pricing/enterprise/legal/home-loop contexts. No "View demo" / "Book a demo" — that variant was tried in the hero and removed.
