...
│   ├── app/sitemap.ts           # dynamic sitemap.xml (static routes + live CMS slugs)
│   ├── app/robots.ts            # origin robots.txt + Sitemap directive (Cloudflare appends it live)
...
Seven trackers wire into `app/layout.tsx` site-wide; do not duplicate per-page. Two use Next.js third-parties (GTM, GA4), two use the Metadata API (GSC + Bing verifications), three are inline IIFE bootstraps in `<head>` via `dangerouslySetInnerHTML` because that's the only way to fire them synchronously before paint (Clarity, Meta Pixel, LinkedIn Insight Tag):
...
- **Meta domain verification** — `metadata.verification.other['facebook-domain-verification'] = 'jun19086bk3h816jakt4xojw9cq3vm'` (added 2026-07-07, PR #47), emitting `<meta name="facebook-domain-verification" …>`. Verifies `clovion.ai` to the Meta pixel business (needed for Aggregated Event Measurement / iOS). **Verify against `www.clovion.ai`** (apex has no DNS — see the "Apex is NOT bound to Railway" note). Same `verification.other` object as Bing.
- **Sitemap + robots (Next.js native routes)** — `app/sitemap.ts` (dynamic `MetadataRoute.Sitemap`, `revalidate=300`) lists 34 static routes + live CMS slugs pulled via `listContent()` for BLOG/NEWS/WEBINAR/RESOURCE/FAQ (excludes the `/features` redirect + `noIndex` posts). `app/robots.ts` emits the origin robots policy + `Sitemap: https://www.clovion.ai/sitemap.xml`. **Live `/robots.txt` is Cloudflare-managed** and *appends* the origin file, so the `Sitemap:` line surfaces publicly. Both live (PR: `feat/seo-sitemap-robots`). Submit the sitemap in GSC/Bing against the **www** property.
...
- **Meta Pixel** — pixel id `1059601186732604`, inlined in `<head>` via `metaPixelBootstrap` constant. Standard `!function(f,b,e,v,...)` IIFE that builds the `window.fbq` queue, async-loads `https://connect.facebook.net/en_US/fbevents.js`, then fires `fbq('init', ...)` + `fbq('track', 'PageView')`. Matching `<noscript><img src="https://www.facebook.com/tr?id=1059601186732604&ev=PageView&noscript=1" />` sits at the top of `<body>` for JS-disabled clients. **SPA tracking**: `RouteTracker.tsx` fires `fbq('track', 'PageView')` on `usePathname()` changes (skips the first render to avoid double-counting the bootstrap's initial fire). To verify in the wild, install the Meta Pixel Helper Chrome extension — green badge = PageView firing. **Standard conversion events (Lead / InitiateCheckout / Schedule) are NOT in code** — they're fired from **GTM** (container `GTM-WHCPZS4P`) as Custom HTML tags calling `fbq('track', …)` on the existing `ce - *` Custom Event triggers (`ce - form_submit`, `ce - get_free_score`, `ce - start_trial`, `ce - pricing_click`, `ce - book_demo`). GTM is the central hub (also fires GA4 + Google Ads `AW-18249357902`). The inline base pixel stays in code; do NOT re-init the pixel in GTM (would double-count PageView). **Conversions API (server-side) IS built now** — see "Meta Conversions API (CAPI)" below.

**Meta Conversions API (CAPI) — server-side conversions with Pixel dedup.** Sends a server copy of each lead conversion straight to Meta's Graph API, deduplicated against the browser Pixel via a shared `event_id`, so conversions lost to iOS/Safari privacy + ad blockers are still counted and Event Match Quality improves.
- **`lib/meta/capi.ts`** — `sendMetaConversion({ req, eventName, eventId, email, firstName, lastName })`: SHA-256-hashes email/name, pulls client IP (`x-forwarded-for`), user-agent, and `_fbp`/`_fbc` cookies, POSTs to `graph.facebook.com/v21.0/1059601186732604/events`. **No-ops when `META_CAPI_TOKEN` is unset and never throws** — it can't break the lead flow it rides on. Honors `META_CAPI_TEST_CODE` (Events Manager Test Events). Accepts a plain `Request` so it works in both NextRequest and Request handlers.
- **`lib/analytics.ts`** — `newMetaEventId(prefix)` generates the shared id (client-side, at submit); `formSubmit(name, location, metaEventId?)` now carries `meta_event_id` on the `form_submit` dataLayer event.
- **Dedup handshake**: forms generate the id at submit, push it via `formSubmit` to the dataLayer, AND include `metaEventId` in the POST body. The GTM **"Meta Pixel - Lead (form submit)"** Custom HTML tag (trigger `ce - form_submit`) reads it as `{ eventID: '{{DLV - meta_event_id}}' }` (DLV variable's Data-Layer name = `meta_event_id`), so the browser Lead and server Lead share the id and Meta counts one.
- **Wired forms**: `LeadCaptureModal`, `free-score/EmailGate`, `home/NewsletterPopup`, `tools/shared/ToolLeadModal`, `resources/[slug]/LeadGate`. **Wired routes** (fire `Lead` on success): `/api/lead`, `/api/tool-lead`, `/api/newsletter`, `/api/resource-lead`. **`/api/scan` intentionally skipped** — it's the product action after the Lead already fired; sending it would double-count.
- **Env**: `META_CAPI_TOKEN` is set in Railway (prod) + `site/.env.local` (dev, git-ignored). Read from env only, never committed.
- **LinkedIn Insight Tag** — partner id `9552940`, inlined in `<head>` via `linkedInBootstrap` constant. Sets `window._linkedin_partner_id`, pushes it into `window._linkedin_data_partner_ids`, then async-loads `https://snap.licdn.com/li.lms-analytics/insight.min.js` which fires the initial beacon to `px.ads.linkedin.com/wa/`. Matching `<noscript><img src="https://px.ads.linkedin.com/collect/?pid=9552940&fmt=gif" />` is in `<body>` alongside the Meta Pixel noscript. **SPA tracking**: `RouteTracker.tsx` also fires `lintrk('track', { conversion_id: '' })` on route change (the Insight Tag doesn't auto-track virtual pageviews otherwise; empty `conversion_id` is the canonical "just re-fire the base pixel" form — pass a real id later when conversions get wired). LinkedIn Campaign Manager → Account Assets → Insight Tag flips to **Active** within ~15 min of first real production traffic.
...
**SPA route tracking — `site/components/RouteTracker.tsx`** — client component mounted in the root layout. On `usePathname()` changes, fires three beacons in sequence: GA4 `page_view` (via `analytics.pageView`), Meta Pixel `fbq('track','PageView')`, and LinkedIn `lintrk('track', { conversion_id: '' })`. Skips the first render so it doesn't triple-count alongside the bootstrap fires (GTM's own pageview + Meta Pixel bootstrap PageView + LinkedIn Insight Tag's initial beacon all land on initial load). When adding a new pixel-style tag, extend this hook's effect so SPA navs stay tracked.
...
| Pricing agency band "Talk to sales" | `components/pricing/FeatureContent.tsx` | `pricing_agency` (the light-rebuild compare table has NO CTAs — the old `pricing_table` callsite is gone) |
...
| Talk-to-Sales buttons (pricing tier + pricing agency band + legal + comparison/alt/feature CTABanners) | see "Talk-to-Sales → Calendly" section below | `pricing_card`, `pricing_agency`, `legal_terms`, `final_cta` |
...
**Callsites (updated 2026-07-07 for the pricing light rebuild — the old `pricing_table` / `pricing_faq` / `pricing_final_cta` callsites were removed when the pricing page was rebuilt; the agency-band `pricing_agency` is new. `/affiliate` no longer uses Calendly — its "Join the Program" CTAs go to `https://clovion.tolt.io/login` and "Talk to us" is a `mailto:`):**
...
| /pricing agency band "Talk to sales" | `components/pricing/FeatureContent.tsx` | `openCalendly('pricing_agency')` |
...
- **P4 — FAQ centered single-column**: every FAQ section across the site renders via the shared `<FAQAccordion>` component (`site/components/FAQAccordion.tsx`) — centered single-column, ~720–800px max-width, all-closed by default, multi-open rows, plus-icon rotating to × on open. No grid template needed — it stacks fine on mobile because it's already single-column. The universal headline is "Frequently Asked Questions" (user-locked).
...
**Chrome mobile specifics**: the desktop mega-menu is `hidden lg:block` (mobile uses the drawer); its width is conditional on child count — `w-[min(920px,calc(100vw-2rem))]` 3-col grid for long menus (Features), `w-[min(480px,calc(100vw-2rem))]` single-column for short ones (Resources). Mobile drawer has `max-h-[calc(100vh-4rem)] overflow-y-auto overscroll-contain`. Footer columns default `grid-cols-1 sm:grid-cols-2 md:grid-cols-4`. Sticky header pads with `pt-[env(safe-area-inset-top)]` for the iOS notch. Hamburger is 44×44px tap target.
...
- `pricingTiers` (4: Free/Starter/Growth/Enterprise) — **STALE**, no longer drives `/pricing`. The light rebuild hardcodes 3 tiers inside `components/pricing/PricingTiers.tsx`: Starter **$119** (launch $95), Growth **$319** (launch $255, "Most popular"), Enterprise Custom. `addOns`
...
**Marketing surface — 28+ routes on `main`. LIGHT (#FAF9F7 + Clove orange) is now the default; only `/customers`, `/about`, `/changelog` + the `/news`, `/webinars`, `/faq`, `/compare`, `/alternatives`, `/docs`, `/legal` prefixes stay dark (with per-page light exceptions). See the "Dark theme" section for the exact current sets. Route list:**
...
/features/ai-visibility-tracking               (LIGHT — components/ai-visibility/*. Converted dark→light PR #69; removed from dark machinery.)
/features/geo-improvement-suggestions          (LIGHT — components/geo/*. Converted dark→light PR #69.)
/features/sentiment-analysis                   (LIGHT — components/sentiment/*. Converted dark→light PR #69. Nav label now reads "Sentiment Analysis" — was previously "Brand Perception" until a dedicated brand-perception page was built; URL slug unchanged)
/features/brand-perception                     (LIGHT — components/brand-perception/*. Converted to a light editorial rebuild 2026-07-07 (PR #42), removed from dark machinery. Distinct from sentiment-analysis — that one tracks positive/neutral/negative tone, brand-perception extracts the WHAT — ease of use, audience fit, pricing perception, maturity, etc.)
/features/brand-audit                          (LIGHT — components/brand-audit/*. Added 2026-07-07 (PR #43). HeroScan / FourAreas / FindingCard / AuditComparison / FreshnessTimeline / AuditToAction / RecommendationTypes + primitives.tsx.)
/features/recommendation-engine                (LIGHT — components/recommendation/*. Added 2026-07-07. HeroContrast / GapCards / InsightToImprovement / MonitoringMotif / OutcomePanel / RecommendationCard + primitives.tsx.)
/features/fanout-query                         (LIGHT — components/fanout/*. Converted dark→light PR #69.)
/features/ai-crawlability                      (LIGHT — components/crawl/*. Converted dark→light PR #69.)
/features/platform-coverage                    (REMOVED 2026-07-08 — page deleted; dropped from nav mega-menu + all machinery)
/pricing                                       (LIGHT #FAF9F7 — components/pricing/*. Rebuilt 2026-07-07 (PR #41): launch banner, Monthly/Annually billing toggle with a DYNAMIC measured slider, 3 tiers Starter $119 (launch $95) / Growth $319 (launch $255, "Most popular") / Enterprise Custom, agency band, compare table, FAQ. 2026-07-08: emerald accent remapped to Clove orange `#C2410C` by overriding `--positive`/`--positive-bg`/`--positive-border` on the wrapper (flips Launch-offer + Most-popular badges, Growth border, discount text, checkmarks, "2 months free" pill); ComparisonTable content replaced with 16 new rows (AI Engine → Integrations, ✕ for not-included); NEW `OurPartners` section (Unilever/Canon/DHL/Netpower/Reckitt via color-mask, above the FAQ); hero sized to one-line headline + two-line subtitle.)
/affiliate                                     (LIGHT #FAF9F7 — components/affiliate/*. Rebuilt 2026-07-07 (PR #45): added Commission (30%→50% meter) / Three Tiers (Individual/Mid-Tier/Agency) / Featured Partners (seal emblem) / mid-CTA sections ABOVE How-It-Works. All "Join the Program" CTAs → https://clovion.tolt.io/login; "See commission rates" smooth-scrolls to #commission-rates. Removed from dark machinery.)
/blog                                          (LIGHT — components/blog/*, ChipRail filter + featured card + post grid + newsletter band + FAQ. Whole /blog + /resources content system is LIGHT; not a dark prefix.)
/blog/category/geo                             (LIGHT — same composer with initialCategory='geo')
/blog/category/ai-search                       (LIGHT — same composer with initialCategory='ai-search')
/blog/category/seo                             (LIGHT — same composer with initialCategory='seo')
/compare                                       (DARK — hub)
/compare/clovion-vs-profound                   (DARK)
/compare/clovion-vs-peec-ai                    (LIGHT — LIGHT_EXCEPTION. Rebuilt 2026-07-08 (PR #61) on new content: centered hero → dual Measure/Recommend flow (Peec: Measure→Benchmark→Recommend / Clovion: Measure→Diagnose→Recommend→Prove) → 11-row feature table → engine coverage → side-by-side pricing → when-to-choose → FAQ → orange CTA → verified note. Clove orange accent, ink-neutral Peec side. Its DeepDive.tsx was deleted.)
/compare/clovion-vs-otterly                    (LIGHT — LIGHT_EXCEPTION. Otterly comparison; has its own DeepDive.tsx)
/compare/clovion-vs-searchable                 (LIGHT — LIGHT_EXCEPTION. Searchable comparison; has its own DeepDive.tsx)
/alternatives/profound
/customers                                     (DARK — components/customers/*, LogoWall hero mock + filter grid + testimonials + FAQ)
/docs
/docs/getting-started
/free-ai-visibility-score                      (LIGHT #FAF9F7 — components/free-score/*, interactive scan via /api/free-score. Converted dark→light 2026-07-09 (PR #70); removed from dark machinery. Light + Clove-orange wrapper; final-CTA is a self-scoped .clv-dark band.)
/tools/llms-txt-generator                      (LIGHT #FAF9F7 — components/tools/llms-txt-generator/*. Free tool. Converted dark→light 2026-07-09 (PR #70).)
/tools/ai-crawlability-checker                 (LIGHT #FAF9F7 — components/tools/ai-crawlability-checker/*. Free tool. Converted 2026-07-09 (PR #70).)
/tools/robots-checker                          (LIGHT #FAF9F7 — components/tools/robots-checker/*. Free tool. Converted 2026-07-09 (PR #70).)
/tools/fanout                                  (LIGHT #FAF9F7 — components/tools/fanout/*. Free tool. Converted 2026-07-09 (PR #70).)
/about
/changelog
/legal/privacy
/legal/terms
...
The feature pages all share the same architecture: a server `page.tsx` exports `Metadata` + embeds 1–2 JSON-LD blocks (FAQPage + optionally SoftwareApplication) + wraps a client `FeatureContent` composer in a scope div. Dark pages use `<div className="clv-dark clv-ai-vis-page">`; **light pages (all feature pages, pricing, affiliate, the 5 FREE TOOLS pages, etc.) use `<div className="clv-ai-vis-page">` with NO `clv-dark`** (light pages typically also set `--bg: #FAF9F7` + the Clove-orange `--positive` remap inline — see "FREE TOOLS pages" note) — the `.clv-ai-vis-page` class is just the scope token for Hanken Grotesk paragraphs (shared across dark AND light feature/pricing/affiliate pages; the name is historical). To flip a page dark→light: drop `clv-dark` from the wrapper AND remove the route from all three machinery spots (see "Adding a new dark route" — do it in reverse).
...
- `components/Header.tsx` + `components/Footer.tsx` — LIGHT variants for non-dark routes. **"Features" is a `<button>` (not a Link)** — it has no href, exists only to open the mega-menu on hover/focus. Mobile nav renders Features as a non-link section header with its 6 children flat-listed below. **`Footer.tsx` FREE TOOLS column** — the first row is now **"Get Free Score" → `/free-ai-visibility-score`**, placed above llms.txt Generator (and the rest of the free-tools list). It's the canonical entry point from the footer. **Social icons are X/Twitter + LinkedIn only** (GitHub removed, PR #44). **Both footers were restructured 2026-07-08 (PR #62) and now share the SAME `groups` array + layout — keep them in sync.** Columns: Product / Resources (Blog, Guides & Downloads, FAQ — News + Webinars removed) / Free Tools / Company (About, Affiliate Program — **no Compare**; Customers temporarily hidden) / Legal / **Comparisons (vs Peec AI, vs Otterly, vs Searchable)**. The brand column also carries an **`<AskAiSummary variant="light|dark" />`** block (`components/AskAiSummary.tsx` — "Ask AI for a summary" CTA) and a **"Follow Us On"** heading above a shared **`<SocialLinks variant="light|dark" />`** (`components/SocialLinks.tsx`, X/Twitter + LinkedIn).
- `components/HomeHeader.tsx` + `components/HomeFooter.tsx` — DARK variants. Same nav-button + 3-col-mega-menu treatment. `HomeFooter` mirrors `Footer`'s `groups` exactly (dark styling only). Keep the two footers in sync when editing links.
- `components/ThemeShell.tsx` — tiny client gate. On `usePathname()` change, toggles `.clv-dark` on `document.documentElement` based on `DARK_ROUTES` (`/customers`, `/about`, `/changelog`) plus a `DARK_PREFIXES` scan (`/news`, `/webinars`, `/faq`, `/compare`, `/alternatives`, `/docs`, `/legal`), minus `LIGHT_EXCEPTIONS` (checked first). **`/blog`, `/resources`, `/tools` are NOT dark.** Pairs with the inline bootstrap script in `app/layout.tsx`.
...
- **`components/cms/PostHeader.tsx`** — the **canonical** header for every CMS post `[slug]` template (blog + news). Renders, in this fixed standard-editorial order: back-link → eyebrow → title → **byline (date · author, directly under the title)** → **cover/hero image** → excerpt/lead → (body follows in the page). Both `app/blog/[slug]/page.tsx` and `app/news/[slug]/page.tsx` use it. **Never hand-roll a post header, place the cover in a separate later section, or drop the byline below the body** — those were the two bugs this component fixes (cover-below-meta, then byline-below-text). Add new post types via this component so the cover stays under the title.
- **Cover-image contract (shared across all 3 surfaces — featured card + grid card in `components/blog/FeatureContent.tsx`, and the detail hero in `PostHeader.tsx`).** The CMS exposes **no image dimensions**, so every cover uses the SAME frame: a fixed **`aspect-ratio: 16/9`** box + `overflow:hidden` + `<img>` `object-fit:cover` / `object-position:center`, plus an **on-top hairline** (`<span>` absolutely positioned, `boxShadow: inset 0 0 0 1px rgba(255,255,255,0.14)`, `pointer-events:none`) so a dark/near-black/broken cover still reads as a framed region (an inset shadow on the *wrapper* is painted under the full-bleed image and is invisible — the hairline must be a sibling layered above the img). This single contract makes covers: **CLS-free** (the 16:9 box reserves space before load), **height-bounded** (a portrait upload is center-cropped, can't tower or stretch sibling grid cards), and **frame-filling with no letterbox bars**. Covers are landscape heroes, so 16:9-cover shows the whole image with no visible crop. **Do not** switch any cover to `object-fit:contain` (reintroduces bars) or to natural-aspect `height:auto` (reintroduces CLS + portrait blowup). Body/inline images inside `.cms-prose` stay natural-aspect (`width:100%;height:auto`) — they are *content*, never cropped. A genuinely solid-black uploaded cover renders black (correct) — that's a CMS content fix (re-upload), not a code bug.
- **CMS SEO/schema flow** (`app/blog/[slug]/page.tsx`, `app/news/[slug]/page.tsx`): `generateMetadata` maps CMS `seo.metaTitle`/`metaDescription`, `og:*` (image falls back to `coverImageUrl`), and `JsonLd` renders `item.jsonLd` (one valid block; `JsonLd.tsx` returns null when empty). **Canonical always emits** — CMS `canonicalUrl` when set, else the post's own `/blog|news/<slug>` resolved against `metadataBase` (`https://www.clovion.ai` — the www host serves 200; apex 301-redirects, so canonical/OG must point at www). `robots` is `{ index: !noIndex, follow: true }` (de-indexed posts still let crawlers traverse links — don't recouple follow to noIndex). News destructures `item.seo ?? {}` defensively (matches blog).
...
Originally only `/` shipped dark per the 2026-06-19 design handoff, then dark scope expanded — but the trend fully reversed and **LIGHT (#FAF9F7 + Clove orange) is now the overwhelming default; only a handful of routes remain dark.** Routes converted to LIGHT + removed from all dark machinery over time: `/` (2026-07-03); `/features/brand-perception`, `/pricing`, `/affiliate` (2026-07-07); `/features/brand-audit` + `/features/recommendation-engine` (new light pages); the remaining 5 dark feature pages `/features/{ai-visibility-tracking,geo-improvement-suggestions,sentiment-analysis,fanout-query,ai-crawlability}` (PR #69); the whole `/blog` + `/resources` content system; and the 5 **FREE TOOLS** pages `/free-ai-visibility-score` + `/tools/{llms-txt-generator,ai-crawlability-checker,robots-checker,fanout}` (2026-07-09, PR #70 — see "FREE TOOLS pages" note below).

**The current dark set (source of truth, `ThemeShell.tsx` — read the file, don't trust prose):**
- `DARK_ROUTES = ['/customers', '/about', '/changelog']` — the only explicit dark pages left.
- `DARK_PREFIXES = ['/news', '/webinars', '/faq', '/compare', '/alternatives', '/docs', '/legal']` — index + every nested `[slug]` under these stays dark. **`/tools`, `/blog`, and `/resources` are NOT dark prefixes** (all light).
- `LIGHT_EXCEPTIONS = ['/compare/clovion-vs-peec-ai', '/compare/clovion-vs-otterly', '/compare/clovion-vs-searchable', '/legal/privacy', '/legal/terms']` — pages that opt OUT of an otherwise-dark prefix.
`Chrome.tsx` (`HOME_ROUTES`/`HOME_PREFIXES`) and the `app/layout.tsx` `themeBootstrap` OR-chain mirror these three exactly — keep all three spots in sync.

**`LIGHT_EXCEPTIONS` (new pattern, 2026-07-08).** Individual pages can opt OUT of an otherwise-dark prefix. `ThemeShell.tsx` and `Chrome.tsx` both define `const LIGHT_EXCEPTIONS = new Set([...])` checked FIRST (return light), and the `app/layout.tsx` bootstrap OR-chain ends with a matching `&& p!==…` guard for each. Current exceptions: the three comparison pages (peec / otterly / searchable) plus `/legal/privacy` + `/legal/terms` render LIGHT with the light Header/Footer, while the `/compare` hub + `/compare/clovion-vs-profound` (and the rest of `/legal/*`) stay dark. **To make one page in a dark prefix light**: add its exact path to `LIGHT_EXCEPTIONS` in ThemeShell + Chrome AND to the bootstrap `&& p!==...` guard, and don't wrap its `page.tsx` in `clv-dark`.

Don't add to OR remove from `DARK_ROUTES` (or the prefix list) without explicit user authorization for that page. **To convert a whole route light**: drop `clv-dark` from its `page.tsx` wrapper AND remove it from all three spots — `DARK_ROUTES` (`ThemeShell.tsx`), `HOME_ROUTES` (`Chrome.tsx`), and the `themeBootstrap` OR-chain (`app/layout.tsx`).

**FREE TOOLS pages — dark→light migration (2026-07-09, PR #70 + #76).** The 5 footer "FREE TOOLS" pages (`/free-ai-visibility-score`, `/tools/llms-txt-generator`, `/tools/ai-crawlability-checker`, `/tools/robots-checker`, `/tools/fanout`) were converted from `.clv-dark` to the light homepage palette. This is the canonical **light + Clove-orange page pattern**, reusable for any dark→light conversion:
- **Wrapper (each `page.tsx`):** drop `clv-dark`, keep `clv-ai-vis-page`, and apply an inline `style` remapping the accent to orange + pinning the bg — `{ ['--bg']: '#FAF9F7', ['--positive']: '#C2410C', ['--positive-bg']: '#FBEEE7', ['--positive-border']: 'rgba(194,65,12,0.34)', background: '#FAF9F7', color: 'var(--ink)' }` (same orange remap as `/pricing`). Emerald affordances (`--positive`) then render Clove orange automatically.
- **Tokens that DON'T auto-flip (must be rewritten):** `--on-ink*` is `#ffffff` in every scope and `--ink-surface` is `#0a0a0f` (near-black) in every scope — a bare "drop clv-dark" leaves white-on-white text and dark panels. Migration map: `--on-ink*`→`--ink`/`--ink-NN`, `--on-ink-15`→`--line`, `background: var(--ink-surface)`→`var(--white)` (card) or `var(--subtle)` (recessed/code), `rgba(255,255,255,α)`→`var(--line)`/`rgba(10,10,15,α)`, hardcoded dark code hexes (`#0f0f14` etc.)→`var(--subtle)`/`var(--ink)`/`var(--line)`. `--ink`, `--white`, `--subtle`, `--line`, and all `--ink-NN` alphas already resolve correctly on light — leave them.
- **Final CTA stays a self-scoped `.clv-dark` band** (matches the light feature pages `brand-audit`/`recommendation-engine`). Each page's `FinalCTA()` wraps its inner card in `<div className="clv-dark">`; its `--on-ink*` + `--ink-surface` + `rgba(255,255,255,0.06)` dot-grid are intentional there — do NOT convert them.
- **Shared modals** `components/tools/shared/{ToolLeadModal,ToolResultModal}.tsx` converted to light: white-knockout logo PNG replaced with the `HaloMark` SVG lockup (ink), dark input/close-button surfaces `--ink-surface`→`--subtle`. (Free-score `ThankYou` + robots-checker's own local result-modal still reference the white logo PNG rendered dark via CSS `filter` — a real dark-wordmark asset would be a cleaner future swap.)
- **No hero shade (PR #76).** These pages originally carried decorative hero shading (radial corner vignettes, ellipse washes, grid/dot-grid textures, the `HeroShade` component). All were removed so hero + section backgrounds are **entirely solid `#FAF9F7`** like the homepage. Kept: the final-CTA dark bands, solid `--subtle` section tones, and functional gradients (marquee masks, score dials, product-mock internals). When authoring/converting a light page, do NOT add `HeroShade` or radial-gradient background overlays — keep backgrounds flat solid.
...
- `PillarStepper.tsx` — sticky-pinned 4-pillar section. **Scroll-driven** (no wheel hijack — that was ripped out after 5 failed iterations; see commit `503bb12`). Left: vertical pillar list. Right: framed product-UI mocks (`MockVisibility`, `MockPerception`, `MockRankings`, `MockRecommendations`). **Heading lives INSIDE the sticky pin** AND **stays pinned at the top of the viewport** throughout the pillar scroll (not centered) so the H2 + subtitle remain visible above whatever pillar is active.
  ...
  - **All four mocks are now fully coded — JSX + CSS + inline SVG, ZERO raster.** They live under `components/home/mocks/` (the PNGs in `public/home/` and the `next/image` import were deleted). Each is a client component `export function MockX({ show }: { show: boolean })` that fills its 100%×100% full-bleed slot and plays a choreographed reveal when `show` flips false→true. **WHITE-SURFACE dashboards, theme-INDEPENDENT** — the mocks faithfully mimic the product reference screenshots (white cards with dark text and real brand colors), and render that way regardless of `.clv-dark` on the homepage. They achieve this by spreading the `LIGHT` token override from `mocks/palette.ts` onto each mock root, which re-defines `var(--white)`/`var(--ink)`/`var(--line)`/`var(--positive)` etc. to their light values inline — same trick as `.clv-chat-island`. Scale via **container-query units (`cqw`)** — `containerType: 'size'` on each root — so they read correctly at any width.
    ...
    - `mocks/glyphs.tsx` — inline-SVG brand marks (`MondayGlyph`, `PipedriveGlyph`, `SalesforceGlyph`, `ChatGptGlyph`, `DiamondCheck`, `EngineGlyph`, `UserGlyph`). Coloured via the BRAND palette (Monday tri-stripe, Pipedrive green P, Salesforce sky cloud, diamond violet) so they read as authentic product UI inside the white dashboards.
    - `mocks/palette.ts` — fixed LIGHT palette exports: `LIGHT` (the design-token override map), `BRAND` (competitor brand hexes — `mondayRed/Yellow/Green`, `pipedrive`, `salesforce`, `diamond`, `chatgpt`), `RED` + `RED_BG` + `RED_BORDER` + `BLUE` (restrained brights for pills/dots/icons), `TAG_COLORS` (Brand Perception tag chip palette), `HL` (Brand Perception highlight tints — pos green / neg red / neu yellow).
...
- `Testimonials.tsx` — 3-card grid (Mirjam Meling / Erland Bruvik / Morten André Hjelle, all CEO-tier customer attributions). Quotes are long-form here (vs. the short-form testimonials in `lib/content.ts` used by `TestimonialRail` elsewhere).
...
- `HomeFAQ.tsx` + `homeFaqs.ts` — homepage-scoped FAQ section, slotted between `<Loop />` and `<HomeCTA />` in `app/page.tsx`. Renders 15 Q/A from `homeFaqs.ts` by delegating to the shared `<FAQAccordion items={...} />` component (`site/components/FAQAccordion.tsx`) — centered single-column, all-closed by default, multi-open, universal headline "Frequently Asked Questions". FAQPage JSON-LD is embedded directly on `app/page.tsx` so the schema lives alongside the page metadata. Copy is locked — sourced from the user's `Homepage FAQ.pdf` spec.
...
7. **FAQ section**: use `<FAQAccordion items={FAQS} />` imported from `components/FAQAccordion` (define a local `FAQS: { q: string; a: string }[]` array near the top of `FeatureContent.tsx`). The shared component owns the centered single-column layout, all-closed-by-default multi-open behavior, plus-icon rotation, and the universal "Frequently Asked Questions" headline. Do NOT hand-roll FAQ markup or import the old 5fr/7fr sticky pattern — every page (homepage, all 7 dark feature pages, `/pricing`, `/affiliate`, `/customers`, `/free-ai-visibility-score`, the 4 `/tools/*` pages) routes through this single component. The `/blog` page's FAQ stays as inline markup but is the visual reference matching the shared component.
...
**Colors. B&W brand book + a single Clove-orange accent.** Body bg is cream `#FAFAF7`. Surfaces are white. Subtle alt bg is `#F5F3EF`. Line `#eceae5`. Ink `#0a0a0f` plus rgba steps via Tailwind tokens (`ink/80/70/60/50/40/20/10`). Mid-grays via Tailwind's built-in `neutral-*`. **Clove orange `#C2410C`** (the "Clove" wordmark color in `components/home/ClovAgent.tsx`) is the ONE approved chromatic accent — introduced 2026-07-08. Its ramp: `#EA580C` (glow / gradient start), `#C2410C` (primary), `#9A3412` (deep / gradient end / on-white text), `#FBEEE7` (pale tint fill), `rgba(194,65,12,0.22)` (hairline). Used deliberately as an accent (comparison pages' Clovion column + CTAs, pricing `--positive` remap, home ClovAgent) — NOT as a wash. Everything else stays ink-on-cream. **Still no purple, no other hues.** Gradients otherwise monochrome. **Do NOT reintroduce `purple-*` utilities.**
...
- **B&W brand book + one Clove-orange accent (`#C2410C`).** No purple, no other hues. Orange is used sparingly as an accent (see the "Colors" section). **Light theme is the default and the direction of travel** — `/`, `/pricing`, `/affiliate`, `/features/brand-perception`, `/features/brand-audit`, `/features/recommendation-engine`, and the three comparison pages (`/compare/clovion-vs-peec-ai`, `/compare/clovion-vs-otterly`, `/compare/clovion-vs-searchable`) are all LIGHT. `/features/platform-coverage` was **removed** (2026-07-08). **All 5 remaining feature pages went LIGHT (PR #69), `/blog` + `/resources` are LIGHT, and the 5 FREE TOOLS pages (`/free-ai-visibility-score` + `/tools/*`) went LIGHT (PR #70/#76).** Dark is now reserved for only `/customers`, `/about`, `/changelog` + the `/news`, `/webinars`, `/faq`, `/compare`, `/alternatives`, `/docs`, `/legal` prefixes (see "Dark theme — broad coverage" for the exact current `DARK_ROUTES`/`DARK_PREFIXES`/`LIGHT_EXCEPTIONS` sets). The homepage `/` keeps `ChatDemo`/`Loop`/`HomeCTA` as self-scoped dark bands on the light page. Don't add OR remove routes from the dark scope without per-page confirmation.
...
- **Header nav is short.** Features (mega-menu trigger only, NOT a link) / Pricing / Resources. (**Customers is temporarily hidden** — commented out in `lib/content.ts` nav as of 2026-07-08; the `/customers` page still exists.) The Features mega-menu has **8 children** (order per `lib/content.ts`): AI Visibility Tracking / GEO Improvement Suggestions / Recommendation Engine / Fanout Query / AI Crawlability / Sentiment Analysis / Brand Perception / Brand Audit. (Platform Coverage was removed 2026-07-08.) The **Resources** dropdown has 3 children: Blog / Guides & Downloads / FAQ (News + Webinars were hidden from the menu 2026-07-08 — the pages/routes still exist, just unlinked). **Long mega-menus (>6 children, i.e. Features) render as a 3-column grid** (`w-[min(920px,calc(100vw-2rem))] grid grid-cols-3`) so the panel fits a laptop viewport; short menus (Resources) stay single-column `w-[min(480px,...)]`. Applied in both `Header.tsx` (light) and `HomeHeader.tsx` (dark). **Sentiment Analysis and Brand Perception are now distinct pages** — Sentiment Analysis (URL `/features/sentiment-analysis`) tracks positive/neutral/negative tone of AI mentions; Brand Perception (URL `/features/brand-perception`) extracts WHAT AI says about your brand (ease of use, audience fit, pricing perception, maturity, etc.). **Compare and Docs live in the Footer only.** `/features` itself is a `redirect('/')` — direct URL silently lands on home, no 404, no destination page. The Header renders Features as a `<button>` (not a Link) because the nav data has no href on that item.
...
- **`lib/content.ts` data that's now stale:** `pricingTiers` is no longer used by `/pricing` — the new light page hardcodes 3 tiers (Starter $119 / Growth $319 / Enterprise Custom, launch prices $95/$255) in `components/pricing/PricingTiers.tsx`. The visible 9-Q pricing FAQ + the page's FAQPage JSON-LD both derive from `components/pricing/faqs.ts` (single source — `app/pricing/page.tsx` imports `P_FAQS` from it to build the schema, so they can't drift). Don't trust `pricingTiers` for pricing display. Other content.ts data (features pillars, customers, faqs, etc.) is still active.
...
- Editing the PillarStepper mocks → they are **fully coded** under `site/components/home/mocks/` (no images). The four mocks render as WHITE dashboards on the dark homepage by spreading `LIGHT` from `mocks/palette.ts` onto each mock root (same trick as `.clv-chat-island`) — so `var(--ink)`/`var(--white)`/`var(--positive)` etc. inside resolve to their light values, theme-independently. Use inline `style={{ ... 'var(--*)' }}` tokens (never Tailwind `text-ink`/`bg-white`), import animation hooks + the `cb` easing literal from `mocks/motion`, import brand marks from `mocks/glyphs`, and reach for `BRAND`/`RED`/`BLUE`/`TAG_COLORS`/`HL` from `mocks/palette` for product-faithful colors. **Never** put `var(--*)` inside a `transition` shorthand (use `cb`). Sizing uses container-query units (`cqw`); the slot in `PillarStepper` already tracks each pillar's `mockAspect`. Honor `prefers-reduced-motion` (the hooks snap to final).
...
- Editing copy in `components/home/PillarStepper.tsx` → the current H2 + subtitle ("Others show you what's happening. Clovion shows you what to do next." / "AI visibility isn't improved by dashboards. It's improved by action. Clovion turns every insight into prioritized recommendations your team can act on immediately.") were set by the user on 2026-06-29 and are NOT locked — they can be changed on request. (They replaced the older "Everything you need to understand…" / "Millions of buying decisions…" copy.) General rule still holds: don't rewrite copy inside any `h1`/`h2`/`p` across the marketing site unless the user explicitly asks; layout-fit asks ("make it 2 lines") mean change font sizes/maxWidth/lineHeight, not the words.
