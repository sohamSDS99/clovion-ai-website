...
│   ├── public/home/             # static dashboard PNGs powering all 4 PillarStepper pillars — 4 visibility-frame-*.png (Visibility slideshow), brand-perception.png, visibility-ranking.png, opportunities.png
...
- `PillarStepper.tsx` — sticky-pinned 4-pillar section. **Scroll-driven** (no wheel hijack — that was ripped out after 5 failed iterations; see commit `503bb12`). Left: vertical pillar list. Right: framed product-UI mocks (`MockVisibility`, `MockPerception`, `MockRankings`, `MockRecommendations`). **Heading lives INSIDE the sticky pin** AND **stays pinned at the top of the viewport** throughout the pillar scroll (not centered) so the H2 + subtitle remain visible above whatever pillar is active.
  ...
  - **Right column ratio** is `minmax(220px, 0.32fr) 1.68fr` (pillar list / mock) — widened from the original 0.5fr/1.5fr so the mock panel takes ~84% of the grid. **Mock panel height** is `clamp(560px, 68vh, 760px)` — bumped from `clamp(480px, 58vh, 680px)` so the 1.53-aspect images (Competitive Positioning + Recommendations at 1876×926 and 1876×1192 respectively) fill the slot at their natural width-driven height without letterbox. 912-wide slot ÷ 1.53 = 596px required minimum; 68vh @ 900px viewport = 612px, comfortably above.
  ...
  - **All four pillars are now picture-based, full-bleed:**
    - **Pillar 1 (Visibility)** — `mockAspect: '1864 / 1075'`. `MockVisibility` cycles the 4 `visibility-frame-{1..4}-*.png` images (each 1864×1075 — see the dark-edge gotcha below) with a setInterval, hold 800ms, opacity+transform crossfade `0.5s cubic-bezier(0.4, 0.0, 0.2, 1)`, blur bloom 0.4s. Full cycle ~3.2s. Hover pauses; `prefers-reduced-motion: reduce` locks to frame 0. `next/image` with `unoptimized` so the raw bytes are served.
    - **Pillar 2 (Brand Perception)** — `mockAspect: '1834 / 961'`. `MockPerception` renders `/home/brand-perception.png` plus a two-phase reveal: phase 1 — 6 horizontal white strips wipe scaleX(1→0) left-to-right in cascade (220ms stagger, 700ms linear each, simulating typing); phase 2 — color image layer fades in via clip-path bottom inset stepping through 6 values (90/75/56/38/23/0) with 240ms stagger so highlights light up section-by-section in reading order. Animation gated on `show` prop so it fires when pillar becomes active. Honors `prefers-reduced-motion`.
    - **Pillar 3 (Competitive Positioning)** — `mockAspect: '1876 / 926'`. `MockRankings` renders `/home/visibility-ranking.png` (1876×926, trimmed via workflow agent from a 1876×1226 source — content extends to y=925, so the trailing 300px of bottom whitespace was cropped). White wrapper bg so any residual letterbox blends with the dashboard's own white. No animation — static design-spec PNG.
    - **Pillar 4 (AEO/GEO Recommendations)** — `mockAspect: '1876 / 1192'`. `MockRecommendations` renders `/home/opportunities.png` (1876×1192, trimmed from 1876×1226 — 34px of bottom whitespace removed). White wrapper bg same as MockRankings. No animation. This pillar used to be the only CSS-recreated mock; replaced with image to match the other 3 picture-based pillars.
  - **Visibility frame dark-edge gotcha.** The source Figma exports (1920×1280) have a ~22px dark outer frame (RGB 22,22,25). A crop offset of x=20 leaves the last 2px of that frame inside the cropped image, painting a 5–10px black band along each edge. Safe crop offset is `x ≥ 25` / `y ≥ 25` / `width ≤ 1869` / `height ≤ 1224`. The current crop is `1864×1075` from offset (28, 75) — fully past the rim. Always verify edge pixels are RGB 255 after recropping (e.g. `magick FILE -crop 1x100+0+200 -resize 1x1 -format "%[fx:mean.r*255]" info:`).
...
- Editing the PillarStepper picture-based mocks → the source images live in `site/public/home/` (`visibility-frame-{1..4}-*.png`, `brand-perception.png`, `visibility-ranking.png`, `opportunities.png`). Each pillar's `mockAspect` in the `PILLARS` array must match the picture's natural aspect or the card will letterbox/overflow. When recropping, **verify edge pixels are pure white** (see the visibility-frame dark-edge gotcha). Updating mockAspect → also update Next/Image `width`/`height` in the corresponding `Mock*` component. **MockRankings and MockRecommendations have `background: '#ffffff'` on their wrapper divs** so any residual letterbox blends with the dashboards' own white backgrounds — entire frame reads as one continuous white surface.
