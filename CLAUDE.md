...
**Marketing surface — 28+ routes on `main`. The theme mix is shifting toward LIGHT — `/`, `/pricing`, `/affiliate`, and several `/features/*` pages are now light; the remaining feature pages + content-prefix routes stay dark. See the "Dark theme" section for the exact `DARK_ROUTES` set. Route list:**
...
The feature pages all share the same architecture: a server `page.tsx` exports `Metadata` + embeds 1–2 JSON-LD blocks (FAQPage + optionally SoftwareApplication) + wraps a client `FeatureContent` composer in a scope div. Dark pages use `<div className="clv-dark clv-ai-vis-page">`; **light pages (pricing, affiliate, brand-perception, brand-audit, recommendation-engine, platform-coverage) use `<div className="clv-ai-vis-page">` with NO `clv-dark`** — the `.clv-ai-vis-page` class is just the scope token for Hanken Grotesk paragraphs (shared across dark AND light feature/pricing/affiliate pages; the name is historical). To flip a page dark→light: drop `clv-dark` from the wrapper AND remove the route from all three machinery spots (see "Adding a new dark route" — do it in reverse).
...
Originally only `/` shipped dark per the 2026-06-19 design handoff, then dark scope expanded — but the trend has since reversed and routes are being converted BACK to light. **Converted to LIGHT and removed from all dark machinery: `/` (2026-07-03); `/features/brand-perception`, `/pricing`, and `/affiliate` (2026-07-07); plus the new light pages `/features/brand-audit` and `/features/recommendation-engine`.** The current `DARK_ROUTES` set (source of truth, `ThemeShell.tsx`) is exactly: `/features/ai-visibility-tracking`, `/features/geo-improvement-suggestions`, `/features/sentiment-analysis`, `/features/fanout-query`, `/features/ai-crawlability`, `/free-ai-visibility-score`, `/customers`, `/about`, `/changelog`, `/blog`, `/blog/category/{geo,ai-search,seo}` — plus every page under the `/blog/`, `/news/`, `/webinars/`, `/resources/`, `/faq/`, `/compare/`, `/alternatives/`, `/docs/`, `/legal/`, `/tools/` prefixes. (`/features/platform-coverage` was removed from the set when that page was deleted, 2026-07-08.)

**`LIGHT_EXCEPTIONS` (new pattern, 2026-07-08).** Individual pages can opt OUT of an otherwise-dark prefix. `ThemeShell.tsx` and `Chrome.tsx` both define `const LIGHT_EXCEPTIONS = new Set([...])` checked FIRST (return light), and the `app/layout.tsx` bootstrap OR-chain ends with `&& p!=='/compare/clovion-vs-peec-ai' && p!=='/compare/clovion-vs-otterly' && p!=='/compare/clovion-vs-searchable'`. Current exceptions: the three comparison pages (peec / otterly / searchable) render LIGHT with the light Header/Footer, while the `/compare` hub + `/compare/clovion-vs-profound` stay dark. **To make one page in a dark prefix light**: add its exact path to `LIGHT_EXCEPTIONS` in ThemeShell + Chrome AND to the bootstrap `&& p!==...` guard, and don't wrap its `page.tsx` in `clv-dark`.
...
- **B&W brand book + one Clove-orange accent (`#C2410C`).** No purple, no other hues. Orange is used sparingly as an accent (see the "Colors" section). **Light theme is the default and the direction of travel** — `/`, `/pricing`, `/affiliate`, `/features/brand-perception`, `/features/brand-audit`, `/features/recommendation-engine`, and the three comparison pages (`/compare/clovion-vs-peec-ai`, `/compare/clovion-vs-otterly`, `/compare/clovion-vs-searchable`) are all LIGHT. `/features/platform-coverage` was **removed** (2026-07-08). Dark is now reserved for the remaining feature pages (ai-visibility-tracking, geo-improvement-suggestions, sentiment-analysis, fanout-query, ai-crawlability), `/free-ai-visibility-score`, `/customers`, `/about`, `/changelog`, and the content-prefix routes (see "Dark theme — broad coverage" for the exact `DARK_ROUTES` set + light exceptions). The homepage `/` keeps `ChatDemo`/`Loop`/`HomeCTA` as self-scoped dark bands on the light page. Don't add OR remove routes from the dark scope without per-page confirmation.
